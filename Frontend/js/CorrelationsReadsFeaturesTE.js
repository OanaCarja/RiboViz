...
function change8() {
	...
	var year8RNA=d3.select("#yearform2").node().value;
	var author8RNA=d3.select("#authorform2").node().value;
	var thedataset8RNA=d3.select("#dataform2").node().value;
	var string="../Data/";

	var thefile=string.concat("F7_",year8,"_",author8, "_", thedataset8,".tsv");
	var thefileRNA=string.concat("F7_",year8RNA,"_",author8RNA, "_", thedataset8RNA,".tsv");
...
function analyze(error, data, data1) {
  ...
  var  dataRPF = data.map(function(d) { return Math.pow(10,d.RPF); });
  var  dataRNA = data1.map(function(d) { return Math.pow(10,d.RNA); }); 
  var te =dataRPF.map(function(n, i) { return n / dataRNA[i]; });
 
 
 // xFigure7.domain(d3.extent(te));
...
var  data = data.map( function (d, i) { 
	...
	//xFigure7.domain([0, d3.max(data, function(d) { return d.d1; })]);
  	yFigure7.domain(d3.extent(data, function(d) { return d.d2; }));
  	//yFigure7.domain([0, d3.max(data, function(d) { return d.d2; })]);
...
updateFigure7(data, "RPF", d3.select('input[name="features"]:checked').node().value); 
